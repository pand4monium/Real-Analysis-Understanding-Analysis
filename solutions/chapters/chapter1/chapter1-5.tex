...
\bx{ % Exercise 1.5.9
    A real number $x \in \mathbb{R}$ is called \textit{algebraic} if there exist integers $a_0, a_1, a_2, \dots, a_n \in \mathbb{Z}$, not all zero, such that \[
        a_nx^n + a_{n-1}^{n-1}+ \dots + a_1x+a_0 = 0.
    \]Said another way, a real number is algebraic if it is the root of a polynomial with integer coefficients. Real numbers that are not algebraic are called \textit{transcendental} numbers. Reread the last paragraph of Section 1.1. The final question posed here is closely related to the question of whether or not transcendental numbers exist. \ea{
        \item Show that $\sqrt{2}$, $\sqrt[3]{2}$ and $\sqrt{3} + \sqrt2$ are algebraic.
        \item Fix $n \in \mathbb{N}$ and let $A_n$ be the algebraic numbers obtained as roots of polynomials with integer coefficients that have degree $n$. Using the fact that every polynomial has a finite number of roots, show that $A_n$ is countable.
    ...
    \sol{\ea{
        \item The first two are trivial as they are roots of $x^2-2=0$ and $x^3-2=0$. For $\sqrt3+\sqrt2$, we can start working backwards: \begin{align*}
            x = \sqrt3 + \sqrt2 & \implies x^2 = 5  + 2 \sqrt6 \\
            & \implies (x^2 - 5)^2 - 24 = 0,
        \end{align*}thus giving a polynomial, hence $\sqrt2 + \sqrt3$ is algebraic.
        \item We need to show that $A_n \sim \mathbb{Z}^n \sim \mathbb{N}^n \sim \mathbb{N}$ and hence countable. \er{
            \item $A_n \sim \mathbb{Z}^n$ since choosing $n$ integer coefficients for a polynomial of degree $n$ is equivilant to an ordered list of $n$ integers.
            \item $\mathbb{Z}^n \sim \mathbb{N}^n$ since $f: \mathbb{N}^n \rightarrow \mathbb{Z}^n$ is just the piecewise application of $g: \mathbb{N} \rightarrow \mathbb{Z}$.
            \item $\mathbb{N}^n \sim \mathbb{N}$ since it is the intersection of finite sets $\bigcup^\infty_{k=1} \{(x_1, \dots, x_n): x_1 + \dots + x_n = k\}$.
        }
        \item The set of algebraic numbers is countable as it is the union of countable sets $\bigcup^\infty_{n=1} A_n$. The set of transcendental numbers is uncountable, otherwise, it implies that the set of real numbers is countable, which is not possible.
    }}
}

\bx{ % Exercise 1.5.10
    \ea{
        \item Let $C \subseteq [0,1]$ be uncountable. Show that there exists $a \in (0,1)$ such that $C \cap [a, 1]$ is countable.
    ...
    \sol{\ea{
        \item \AFSOC $a$ does not exists, so $C \cap [a, 1]$ is countable for all $a \in (0,1)$. Therefore $C = \bigcup_{n=1}^\infty C \cap [1/n, 1]$ is also countable, which is a contradiction. Hence, there exist a $a \in (0,1)$ such that $C \cap [a, 1]$ is uncountable.
        \item Since $\alpha = \sup A$, for any $\epsilon > 0$, $C \cap [\alpha + \epsilon, 1]$ is countable. Therefore, $C \cap [\alpha, 1] = {\alpha} \cup \bigcup_{n=1}^\infty C \cap [1/n,1]$ is countable.
        \item No. Consider $C = \{ 1/n: n \in \mathbb{N}\}$. It has $C \cap [\alpha, 1]$ finite for every $\alpha$, but $C \cap [0,1]$ is infinite.
    }}
}

\bx{ % Exercise 1.5.11
    \textbf{(Schröder-Bernstein Theorem)} Assume there exists a 1-1 function $f: X \rightarrow Y$ and another 1-1 function $g: Y \rightarrow X$. Follow the steps to show that there exists a 1-1, onto function $h: X \rightarrow Y$ and hence $X \sim Y$.

    The strategy is to partition $X$ and $Y$ into components \[
        X = A \cup A' \quad \text{and} \quad Y = B \cup B'
    \] with $A \cap A' = \emptyset$ and $B \cap B' = \emptyset$, in such a way that $f$ maps $A$ onto $B$, and $g$ maps $B'$ and $A'$. \ea{
        \item Explain how achieving this would lead to a proof that $X \sim Y$.
        \item Set $A_1 = X\backslash g(Y)=\{x\in X: x \notin g(Y)\}$ (what happens if $A_1 = \emptyset$?) and inductively define a sequence of sets by letting $A_{n+1} = g(f(A_n))$. Show that $\{A_n: n \in \mathbb{N}\}$ is a pairwise disjoint collection of subsets of $X$, while $\{f(A_n): n\in \mathbb{N}\}$ is a similar collection in $Y$.
        \item Let $A = \bigcup^\infty_{n=1}A_n$ and $B=\bigcup^\infty_{n=1} f(A_n)$. Show that $f$ maps $A$ onto $B$.
        \item Let $A'=X \backslash A$ and $B'=Y\backslash B$. Show $g$ maps $B'$ onto $A'$.
    }
    \sol{\ea{
        \item $f: A \rightarrow B$ and $g: B' \rightarrow A'$ are bijective, therefore we can define \[
        h(x) = \begin{cases}
            f(x) & \text{if } x \in A \\
            g^{-1}(x) & \text{if } x \in A'
        \end{cases}\] which is bijective.
        \item If $A_1 = \emptyset$, we are done as it implies $g(Y) = X$ so $X \sim Y$
        
        Since $A_n \subseteq X$, then $f(A_n) \subseteq Y$, hence $A_n \cap A_1 = \emptyset$ for all $n \in \mathbb{N}$ as $A_1 = X \backslash g(Y)$.

        Note that $g \circ f$ is bijective implies that $g(f(A \cap B)) = g(f(A)) \cap g(f(B))$. We will show that by inclusion both ways. Let $h$ be an arbitrary bijective function. \er{
            \item Let $y \in h(A \cap B)$, so there exists $x \in A \cap B$ such that $h(x) = y$, then $x \in A$ and $x \in B$. This implies that $h(x) \in h(A)$ and $h(x) \in h(B)$, hence $y = h(x) \in h(A) \cap h(B)$.
            \item Let $y \in h(A) \cap h(B)$, so $y \in h(A)$ and $y \in h(B)$, then there exist $x \in A$ and $x \in B$ such that $h(x) = y$. This implies $x \in A \cap B$, hence $y \in h(A \cap B)$.
        }

        Now to show pairwise disjointment for all subsets, consider $i,j \in \mathbb{N}$, with $i<j$. \[
            A_j \cap A_i = g(f(A_j \cap A_i)) = g(f(A_{j})) \cap g(f(A_i)) = A_{j-1} \cap A_{i-1} = \dots = A_{j-i+1} \cap A_1 = \emptyset
        \] Similarly, $\{f(A_n): n \in \mathbb{N}\}$ is also pairwise disjoint as \[
            A_j \cap A_i = \emptyset \implies f(A_j) \cap f(A_i) = f(A_j \cap A_i) = f(\emptyset) = \emptyset
        \]

        \item $f(A) = B$ as $f(\bigcup^\infty_{n=1} A_n) = \bigcup_{n=1}^\infty f(A_n) = B$. ($B$ is defined to be the range of $f$.)
        \item To show that $g(B') = A'$, we will show inclusion both ways: \er{
            \item $g(B') \subseteq A'$. \AFSOC $g(b') \in A$, then since $A_1 \cap g(Y) = \emptyset$, $g(b') \notin A$, implying $g(b') \in \bigcup^\infty_{n=2} A_n = g(B)$, meaning $\exists \vspace{1ex} b \in B$ such that $g(b) = g(b')$, contradicting the fact that $g$ is 1-1.
            \item $g(B') \supseteq A'$. \AFSOC $\exists \vspace{1ex} a' \in A'$, with $a' \notin g(B')$. Since $A' \subseteq g(Y)$, we have $a' \in g(B)$, since $a' \notin g(B')$, and $g(B) \subseteq A$ contradicting $a' \in A$.
        }
    }}
}