...
\bx{ %3.2.10
    ...
    \noindent \textbf{Note:} Words in bold are not in the original question. The word "infinite" is added as any set $\{x\}$ with a single element satisfies the condition in (i) but goes against the spirit of the question.
    }
}
...
\bx{ % 3.2.11
    ...
    \sol{\ea{
        \item Let $L$ be the set of limit points of $A \cup B$ and let $L_a$, $L_b$ be the set of limit points for $A$ and $B$ respectively.
        
        Let $x \in L$, thus there exists a converging sequence $(x_n) \rightarrow x$ that s contained within $A \cup B$. Since $(x_n)$ has infinite terms, there exists a subsequence $(x_{n_k})$ where every term is in $A$ or $B$. Thus this limit $\lim x_{n_k} = x$ must also be a limit point of $A$ or $B$, meaning $x \in L_a \cup L_b$. This shows that $\overline{A \cup B} \subseteq \overline{A} \cup \overline{B}$.

        Now let $x \in L_a$ (WLOG). There exists a sequence $(x_n) \rightarrow x$ that is entirely contained within in $A$. Now since $x_n \in A$, then $x_n \in A \cup B$, so $x \in L$, showing $\overline{A} \cup \overline{B} = \overline{A \cup B}$ completing the proof.

        \item This does not extend to infinity. Consider $A_n = \{1/n\}$, then $\bigcup A_n = \{1/n : n \in \mathbb{N}\}$. However, $\bigcup \overline{A_n} = \bigcup A_n = \{1/n ; n \in \mathbb{N}\} \neq \overline{\bigcap A_n} = \{1/n: n \in \mathbb{N}\} \cup \{0\}$.
    }}
}
...
\bx{ % 3.2.12
    ...
    \sol{
        Consider $B_1 = \{x \in \mathbb{R} : (-\infty, x) \cap A \text{ is uncountable}\}$. $B_1$ must be nonempty; otherwise $A = \bigcup_{n=1}^\infty (-\infty, n) \cap A$ is a union of countable sets, which means that $A$ is countable. Note that if $x \in B_1$ and $y > x$, then $y \in B_1$. Moreover, $\exists \epsilon>0$ such that $x - \epsilon \in B_1$; we can prove this by contradiction. If there exists no such $\epsilon$, then $(-\infty, x - 1/n) \cap A$ must be countable for all $n \in \mathbb{N}$. Since the union of countable sets is countable, then \[
            \bigcup_{n=1}^\infty (-\infty, x - 1/n) \cap A = (-\infty, x) \cap A
        \]is countable, thus a contradiction.

        Similarly, we define $B_2 = \{x \in \mathbb{R}: (x, \infty) \cap A \text{ is countable}\}$ which possesses similar properties to $B_1$.

        Note that $B_1 \cup B_2 = \mathbb{R}$ as if $(- \infty, x) \cap A$ is countable, then $(x, \infty) \cap A$ must be uncountable and vice verse, hence $x \notin B_1 \implies x \in B_2$ and $x \notin B_2 \implies x \in B_1$. So $B = B_1 \cap B_2 \neq \emptyset$. Since both $B_1$ and $B_2$ are both open, $B$ is also open.
   }
}
...
\bx{ % 3.2.13
    ...
    \sol{
        Let $A$ be a set that is both open and close, such that $A \neq \emptyset$. \AFSOC $A \neq \mathbb{R}$, then we can find an $r \in \mathbb{R}$ satisfying $r \notin A$. Since open sets cannot contain its infimum and supremum but closed sets contain its infimum and supremum, we can deduce $A$ is unbounded.

        Consider $A^* = (-\infty, r) \cap A$. $A^*$ is open as it is the intersection of two open sets. $A^*$ is also closed as it is equivalent to $(-\infty, r] \cap A$ which is the intersection of closed sets; this is true as $r \notin A$. Since $A^*$ is unbounded below, $A^* \neq \emptyset$.

        Now for the contradiction. Let $s = \sup A^*$. Since $A^*$ is closed and bounded above, $s \in A^*$. However, w can find $\epsilon>0$, with $V_\epsilon(s) \subseteq A^*$ since $A^*$ is open, contradicting the fact that $s$ is the supremum. 

        Therefore, $A \neq \emptyset \implies A = \mathbb{R}$. Since the complement of an open set is closed and vice verse, $\emptyset$ is the complement of $\mathbb{R}$, hence being both open and closed.
    }
}

\bx{ % 3.2.14
    A dual notion to the closure of a set is the interior of a set. The \textit{interior} of $E$ is denoted $E ^{\circ}$ and is defined as \[
    ...
    \ea{
        ...
        \item Show that $\overline{E}^c = (E^c)^\circ$, and Similarly that $(E^\circ)^c = \overline{E^c}$.
    }

    \sol{\ea{
        \item \er{
            \item By definition, if $E$ is closed, it contains all limit points of $E$, hence $E = \overline{E}$. Similarly, if $E = \overline{E}$, then $E$ is closed by definition.
            \item By definition, if $E$ is open, any $x \in E$ exists $V_\epsilon(x) \subseteq E$, hence $E = E^\circ$. Similarly, if$E = E^\circ$, then $E$ is open by definition.
        }

        \item $x \in \overline{E}^c$ if and only if $x \notin \overline{E} \implies x \notin E$ and $x$ is not a limit point of $E$; as $E \subseteq \overline{E}$. Similarly, $x \in (E^c)^\circ$ if and only if $x \notin E$ and there exists $V_\epsilon(x) \subseteq E$; as suppose $x \in E$, then $x \notin E^c \supseteq (E^c)^\circ$. Both statements are equivalent as "$x$ is not a limit point of $E$" and "here exists $V_\epsilon(x) \subseteq E$ are the same.
        
        With $\overline{E}^c = (E^c)^\circ$ proven, letting $D = E^c$, gives $(\overline{D^c})^c = ((D^c)^c)^\circ$, taking complement on both sides gives us $\overline{D^c} = (D^\circ)^c$. 
    }}
}  

\bx{ % 3.2.15
    A set $A$ is called an $F_\sigma$ set if it can be written as the countable union of closed sets. A set $B$ is called a $G_\delta$ set if it can be written as the countable intersection of open sets. \ea{
        \item Show that a closed interval $[a, b]$ is a $G_\delta$ set.
        \item Show that the half-open interval $(a, b]$ is both a $G_\delta$ and $F_\sigma$ set.
        \item Show that $\mathbb{Q}$ is an $F_\sigma$ set and the set of irrational $\mathbb{I}$ forms a $G_\delta$ set.
    }
    \sol{
        \ea{
            \item $[a, b] = \bigcap_{n=1}^\infty (a + 1/n, b - 1/n)$.
            \item $(a, b] = \bigcap_{n=1}^\infty (a, b-1/n) = \bigcup_{n=1}^\infty [a - 1/n, b]$.
            \item Let ${r_n}$ be the enumeration of $\mathbb{Q}$. Then \[ 
            \mathbb{Q} = \bigcup_{n=1}^\infty [r_n, r_n]\] Similarly, \[
            \mathbb{I} = \bigcap_{n=1}^\infty \mathbb{R} \backslash [r_n, r_n]\]
        }
    }
}