...
\bx{ % 2.4.8
    ...
    \[\text{(a)}\sum_{n=1}^{\infty}\frac{1}{2^n} \quad \text{(b)}\sum_{n=1}^{\infty}\frac{1}{n(n+1)} \quad \text{(c)} \sum_{n=1}^{\infty}\log\left(\frac{n+1}{n}\right)\]
    (In (c), $\log(x)$ refers to the natural logarithm function from calculus.)
    \sol{\ea{
        \item This is a geometric series, allowing us to use the usal trick to determine $s_n$. \begin{align*}
            s_n & = 1 + \frac12 + \frac14 + \dots + \frac{1}{2^n} \\
            \frac12 s_n & = \frac12 + \frac14 + \dots + \frac{1}{2^{n+1}} \\
            \frac12 s_n - s_n & = \frac1{2^{n+1}} - 1 \implies s_n = \frac{2^{-n-1}-1}{2^{-1}-1}
        \end{align*} As $n$ tends to infinity, \[
        \sum_{n=1}^{\infty}\frac1{2^n} = - 1 + \sum_{n=1}^{\infty}\frac1{2^n} = -1 + \lim_{n\rightarrow\infty}\frac{2^{-n-1}-1}{2^{-1}-1} = 1\]

        \item We can use partial fractions to get  \[
        \frac{1}{n(n+1)} = \frac1n - \frac1{n+1}\]which gives a telescoping sum \[
        s_n = 1 - \frac1{n+1}\] Therefore, \[
        \sum_{n=1}^{\infty}\frac{1}{n(n+1)} = \lim_{n\rightarrow \infty}\left(1 - \frac{1}{n+1}\right) = 1\]
        \item Similar to (b), we can use telescoping sum since \[
        \log\left(\frac{n+1}{n}\right) = \log (n+1) - \log(n)\] Hence, we get \[
        \sum_{n=1}^{\infty} \log \left(\frac{n+1}{n}\right) = \log (n+1)\]which does not converge.
    }}
}

\bx{ % 2.4.9
    Complete the proof of Theorem 2.4.6 by showing that if the series $\sum^\infty_{n=0} 2^nb_{2^n}$ diverges, then so does $\sum_{n=1}^\infty b_n$. Example 2.4.5 may be a useful reference.
    \sol{
        The conditions given state that \seq{b}{n} is decreasing and $b_n \geq 0$ for all $n \in \mathbb{N}$. Suppose $\sum_{n=1}^{\infty} 2^n b_{2^n}$ diverges, let $s_k = \sum_{n=1}^{k} n_n$ and $t_k = \sum_{n=1}^{k} 2^n b_{2^n}$, then \begin{align*}
            s_{2^k} & = b_1 + b_2 + (b_3 + b_4) + \dots + (b_{2^{k-1}+1} + \dots + b_{2^k}) \\
            & \geq b_1 + b_2 + (b_4 + b_4) + \dots + 2^{k-1}b_2^k
        \end{align*} We define $t'_k$ to be our new series $b_1 + b_2 + 2b_4 + \dots + 2^{k-1}b_{2^k}$. Then, \[
        t'_k = \frac12 (b_1 + 2b_2 + 4b_4 + \dots + 2^kb_k) + \frac12 b_1 = \frac12 t_k + \frac12 b_1\] Therefore, \[
        s_{2^k} \geq t'_k \geq \frac12 t_k\]By Order Limit Theorem, since $t_k$ diverges, $s_n$ must also diverge.
    }
}

\bx{ % 2.4.10
    \textbf{(Infinite Products).} A close relative of infinite series is the \textit{infinite product} \[
    \prod_{n=1}^\infty b_n = b_1b_2b_3 \dots\] which is understood in terms of its sequence of \textit{partial products} \[
    p_m = \prod_{n=1}^{m} b_n = b_1b_2b_3 \dots b_m\] Consider the special class of infinite products of the form \[
    \prod_{n=1}^{\infty}(1 + a_n) = (1+a_1)(1+a_2)(1+a_3) \dots, \quad \text{where } a_n \geq 0\] \ea{
        ...
        \item Show, in general, that the sequence of partial products converges if and only if $\sum_{n=1}^{\infty} a_n$ converges. (The inequality $1 _ x \leq 3^x$ for positive $x$ will be useful in one direction.)
    }
    \sol{\ea{
        \item This is a telescoping product as most terms cancel each other \[
        p_m = \prod_{n=1}^{m} \left(1+\frac1n\right) = \prod_{n=1}^{m} \frac{n+1}{n} = \frac21 \cdot \frac32\cdot\frac43\cdot \dots \cdot\frac{m+1}{m} = m+1\] therefore, \seq{p}{m} diverges.

        For the case $a_n = 1/n^2$, \[
        \prod_{n=1}^\infty \left(1 + \frac1{n^2}\right) = \frac21\cdot\frac54\cdot\frac{10}9\cdot\dots\] I conjecture that the series converges as the growth seems slower.
        \item Let $p_m$ be the series of partial product and $s_n$ be the series of partial sums.
        
        $(\Rightarrow)$ We will show this using induction. It is obvious that $p_1 \leq s_1 + 1$. By distributing the terms, we get $p_2 = (1+a_1)(1+a_2) = 1 + a_1a_2 + a_1 + a_2 > a_1 + a_2 + 1 = s_2 + 1$, hence $p_2 > s_2 + 1$. Suppose $p_n > s_n + 1$, then \[
        p_{n+1} = (1+a_{n+1})p_n > (1+ a_{n+1})(s_n + 1) = s_n + s_na_{n+1} + a_{n+1} + 1 > s_{n+1} + 1 \]By Order Limit Theorem, if \seq{p}{m} converge, \seq{s}{n} converges.

        $(\Leftarrow)$ Using the inequality $1+x \leq 3^x$, we get \[
        p_n = (1+a_1)(1+a_2) \dots (1 + a_n) = 3^{a_1} \cdot 3^{a_2} \cdot \dots \cdot 3^{a_n} = 3^{s_n}\] Since $s_n$ converges, it is bounded, hence $s_n \leq M$. Hence we get \[
        p_n \leq 3^{s_n} \leq 3^M\] Monotone Convergence Theorem tells us that \seq{p}{m} converges as it is monotone increasing (since $(1+a_n)\geq 1$) and bounded. 
    }}
}